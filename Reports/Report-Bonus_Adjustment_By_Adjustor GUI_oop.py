...
class Report:
    def __init__(self):
        ...
        self.__SiteQuery = 'SELECT Description,SiteID from Site (Nolock)'
        ReportQuery ="select name from sys.procedures where name like 'REPORT_%' order by name"
        Sites = pd.read_sql_query(self.__SiteQuery, self.__conn).reset_index(drop=True)
        Reports = pd.read_sql_query(ReportQuery, self.__conn)
        print(Reports)
